...
class EnclosureHttp {
  ...
  // 保存当前Axios实例对象
  private static axiosInstance: any = Axios.create(genConfig())
  ...
  // 取消的凭证数组
  private sourceTokenList: Array<any> = []

  // 记录当前这一次cancelToken的key
  private currentCancelTokenKey = ''

  // 当前调用目录前缀
  private urlPrefix = import.meta.env.VITE_BASE_URL

  private beforeRequestCallback: any['beforeRequestCallback'] = undefined

  private beforeResponseCallback: any['beforeResponseCallback'] = undefined

  public get cancelTokenList(): Array<any> {
   ...
   */
  // eslint-disable-next-line class-methods-use-this
  private genUniqueKey(config: any): string {
  ...
  private cancelRepeatRequest(): void {
    ...
    this.sourceTokenList = this.sourceTokenList.reduce<Array<any>>(
      (res: Array<any>, cancelToken: any) => {
  ...
  public request<T>(method: any, url: string, param?: any, axiosConfig?: any): Promise<T> {
  ...
  public post<T>(url: string, params?: T, config?: any): Promise<T> {
    return this.request<T>('post', url, params, config)
  }

  public get<T>(url: string, params?: T, config?: any): Promise<T> {
